		...
		<div class="container">
					    <div class="row my-info-area">
							...
							<div class="col-xl-12 title-div">		
								<br><div class="title-name">Challan</div>						
							</div>
			...
			$informationProduct_Detail = $pdo->query("SELECT distribution_history.*,product_information.name AS product_name,product_information.description AS description,product_information.code AS code  FROM `distribution_history` INNER JOIN product_information ON distribution_history.product_id=product_information.id where  distribution_history.distribution_id='$DocumentData' and distribution_history.deleted_at is NULL");
			$serial=1;
			$amount_sum=0;									
            ...
            while ($rowdataProduct_Detail = $informationProduct_Detail->fetch()){	
                    ...
                    <td text align="left" style="padding-left: 10px;"><?php if(!empty($rowdataProduct_Detail["product_name"])){ echo $rowdataProduct_Detail["product_name"]; }  ?></td>
					<td text align="left" style="padding-left: 10px;"><?php if(!empty($rowdataProduct_Detail["code"])){ echo $rowdataProduct_Detail["code"]; }  ?></td>	
					<td>   <?php if(!empty($rowdataProduct_Detail["description"])){ echo nl2br($rowdataProduct_Detail["description"]); } ?></td>
				...
				$Staff_information_signature = $pdo->query("SELECT employee_information.*,hr_designation.name AS designation_name,distribution_summary.created_at AS distribution_date FROM distribution_summary INNER JOIN employee_information ON distribution_summary.created_by=employee_information.id INNER JOIN hr_designation ON employee_information.designation=hr_designation.id where  distribution_summary.distribution_id='".$DocumentData."' ");
                    while($rowdataStaff_information_signature = $Staff_information_signature->fetch()){
							...
							<img src="Signature/<?php echo $rowdataStaff_information_signature["signature"]; ?>" style="height: 30px;width:100px;">
								...
								<?php } ?>
								<p style="font-size: 20px;;margin:0px;"><?php echo date("d/m/Y", strtotime($rowdataStaff_information_signature["distribution_date"])); ?></p>
