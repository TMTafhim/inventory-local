...
<div class="row">
 ...
 <div class="col-sm-3">
   ...
   <input type="submit" name="view" value="Search" class="btn btn-primary"/>
            ...
            $information = $pdo->query("SELECT SUM(used_quantity) AS distribution_quantity,SUM(used_amount) AS distribution_amount,product_information.name AS product_name,project_information.name AS project_name  FROM `material_used_detail_history` INNER JOIN product_information ON material_used_detail_history.product_id=product_information.id INNER JOIN project_information ON material_used_detail_history.project_id=project_information.id  where material_used_detail_history.deleted_at is NULL $querystatus group by material_used_detail_history.product_id");    
			}else{
		
			    
            $information = $pdo->query("SELECT SUM(used_quantity) AS distribution_quantity,SUM(used_amount) AS distribution_amount,product_information.name AS product_name,project_information.name AS project_name  FROM `material_used_detail_history` INNER JOIN product_information ON material_used_detail_history.product_id=product_information.id INNER JOIN project_information ON material_used_detail_history.project_id=project_information.id where material_used_detail_history.deleted_at is NULL and material_used_detail_history.date='$current_date' group by material_used_detail_history.product_id");    
			} 
