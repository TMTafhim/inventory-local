...
<div class="row">
 ...
 <div class="col-sm-3">
   ...
   <input type="submit" name="view" value="Search" class="btn btn-primary"/>
            ...
            $information = $pdo->query("SELECT SUM(distribution_quantity) AS distribution_quantity,SUM(distribution_amount) AS distribution_amount,product_information.name AS product_name,project_information.name AS project_name  FROM `distribution_history` INNER JOIN product_information ON distribution_history.product_id=product_information.id INNER JOIN project_information ON distribution_history.project_id=project_information.id  where distribution_history.deleted_at is NULL $querystatus group by product_id");    
			}else{
            $information = $pdo->query("SELECT SUM(distribution_quantity) AS distribution_quantity,SUM(distribution_amount) AS distribution_amount,product_information.name AS product_name,project_information.name AS project_name  FROM `distribution_history` INNER JOIN product_information ON distribution_history.product_id=product_information.id INNER JOIN project_information ON distribution_history.project_id=project_information.id where distribution_history.deleted_at is NULL and distribution_history.date='$current_date' group by product_id");    
			} 
