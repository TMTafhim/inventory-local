<?php
$EditData = $pdo->query("select * FROM  project_approval_path_name WHERE id='$DocumentData'");
$rowEditData = $EditData->fetch();

	

$Project_Material_Approval_Info_Path_Name = $pdo->query("SELECT employee_information.*,hr_designation.name AS employee_designation,project_approval_path_inforamtion.id AS project_approval_path_id FROM project_approval_path_inforamtion INNER JOIN employee_information ON project_approval_path_inforamtion.employee_id=employee_information.id INNER JOIN hr_designation ON employee_information.designation=hr_designation.id WHERE project_approval_path_inforamtion.project_id='".$rowEditData["project_id"]."' and approval_path_name='$DocumentData' and project_approval_path_inforamtion.deleted_at is NULL ");	
$rowDataProject_Material_Approval_Info_Path_Name= $Project_Material_Approval_Info_Path_Name->fetch()
?>
  ...
  <form method="post" action="?<?php echo substr($page_title,0,-5)."/".$MenuName."/".$DocumentData."/".$PageStatusCheck; ?>" enctype="multipart/form-data">
        ...
        <div class="form-group"> 
         ...
         <label for="project_id">Path Name<span style="color:#F00;">*</span></label>
		  ...
		  <input type="hidden" class="form-control" name="approval_path_name_previous" placeholder="Path Name" id="approval_path_name_previous" value="<?php echo $rowEditData["approval_path_name"]; ?>">
			<input type="hidden" class="form-control" name="project_id_previous" placeholder="Path Name" id="project_id_previous" value="<?php echo $rowEditData["project_id"]; ?>">	
			
		  <input type="text" class="form-control" name="approval_path_name" placeholder="Path Name" id="approval_path_name" required value="<?php echo $rowEditData["approval_path_name"]; ?>">
         ...
         <label for="project_id">Project Information<span style="color:#F00;">*</span></label>
		  <select name="project_id" id="project_id" class="form-control select2" data-quick-create="project" style="width:100%;" required >
            ...
            <?php
				  ...
				  ?>
            <option <?php if(!empty($rowEditData["project_id"]) && $rowEditData["project_id"]==$rowDataProjectInfo["id"]){ echo "selected"; } ?> value="<?php echo $rowDataProjectInfo["id"]; ?>"><?php echo $rowDataProjectInfo["name"]; ?></option>
            <?php }  ?>
            </select>
